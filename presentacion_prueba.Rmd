...
(
  oc_perc_plot = ggplot(q_2, aes(x = perc_muj, y = `perc_usd`, size = total_usd)) +
    geom_point() +
    geom_vline(xintercept = mean(q_2$perc_muj,na.rm = TRUE), linetype = "dashed", color = "blue", size = 1) +
    geom_hline(yintercept = mean(q_2$perc_usd,na.rm = TRUE), linetype = "dashed", color = "red", size = 1) +
...
# Gráfico de barras para los montos
# Gráfico de barras para los montos
cuad_plot_1 = ggplot(cuad_1[1:10, ], aes(x = reorder(NombreInstitucion, `Monto anual USD_Mujeres`))) +
  geom_bar(aes(y = `Monto anual USD_Mujeres`), stat = "identity", position = "stack", fill = "#FFB6C1", alpha = 1, color = "black") +
    geom_bar(aes(y = `Monto anual USD_Hombres`), stat = "identity", position = "stack", fill = "#87CEEB", alpha = 1, color = "black")+
  labs(title = "OC's por Institución", y = "Cantidad") +
  geom_text(data = cuad_1[1:10, ], aes(label = scales::comma(round(`Monto anual USD_Mujeres`)), y = `Monto anual USD_Mujeres`), vjust = 0.5)+
  geom_text(data = cuad_1[1:10, ], aes(label = scales::comma(round(`Monto anual USD_Hombres`)), y = `Monto anual USD_Hombres`), vjust = 1.5)+
  theme_minimal()+
  coord_flip()+
  ylim(0,max(cuad_1[1:10, ]$total_usd))+
...
cuad_1 = q_2 %>% 
  mutate(media_x=mean(perc_muj,na.rm = TRUE),
         media_y = mean(perc_usd,na.rm = TRUE)) %>% 
  filter(perc_muj> media_x & perc_usd>media_y) %>% 
  arrange(desc(`Cantidad OC_Mujeres`))

# Gráfico de barras para los montos
cuad_plot_2 = ggplot(cuad_1[1:10, ], aes(x = reorder(NombreInstitucion, `Cantidad OC_Mujeres`))) +
  geom_bar(aes(y = `Cantidad OC_Hombres`), stat = "identity", position = "dodge", fill = "#87CEEB", alpha = 1, color = "black") +
  geom_bar(aes(y = `Cantidad OC_Mujeres`), stat = "identity", position = "dodge", fill = "#FFB6C1", alpha = 1, color = "black") +
  labs(title = "OC's por Institución", y = "Cantidad") +
  geom_text(data = cuad_1[1:10, ], aes(label = scales::comma(round(`Cantidad OC_Mujeres`)), y = `Cantidad OC_Mujeres`), vjust = 0.5)+
  geom_text(data = cuad_1[1:10, ], aes(label = scales::comma(round(`Cantidad OC_Hombres`)), y = `Cantidad OC_Hombres`), vjust = 1.5)+
  theme_minimal()+
  coord_flip()+
  ylim(0,max(cuad_1[1:10, ]$total_oc))+
